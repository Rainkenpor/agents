...
const PORT = Number(envs.PORT ?? 3003);
...
httpServer.listen(PORT, () => {
	console.log(`✓ MCP Rancher server → http://localhost:${PORT}/mcp`);
	console.log(`✓ Hooks API          → http://localhost:${PORT}/hooks`);
	console.log(
		`  Instancias: ${envs.INSTANCE_NAMES.length ? envs.INSTANCE_NAMES.join(", ") : "(ninguna — revisa RANCHER_INSTANCES)"}`,
	);
	console.log("  Tools:");
