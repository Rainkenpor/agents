/**
 * MCP Rancher — interfaz pública del módulo
 *
 * Exporta rancherMcp: McpModule para el servidor centralizado, que lo monta en
 * /rancher/mcp y /rancher/hooks*. Soporta múltiples instancias de Rancher
 * (QA, dev, …) configuradas vía la variable JSON RANCHER_INSTANCES.
 */
...
async function handler(
  ...
  try {
		const mcpServer = new McpServer({ name: "mcp-rancher", version: "1.0.0" });
		...
		console.error("[rancher] error:", err);
		if (!res.headersSent) {
...
export const rancherMcp: McpModule = {
	slug: "rancher",
	displayName: "MCP Rancher",
	credentials: [
		{
			key: "RANCHER_INSTANCES",
			required: true,
			description:
				'JSON de instancias de Rancher: {"qa":{"url":"https://dardo-qa.distelsa.net","token":"..."},"dev":{"url":"...","token":"..."}}',
		},
	],
