...
for (const mcp of mcpModules) {
	logger.info(
		`[startup] ${mcp.displayName} → POST /${mcp.slug}/mcp  (${mcp.tools.length} tools) (${mcp.hooks?.length ?? 0} hooks)`,
	);
}
		...
		for (const mcp of mcpModules) {
			...
			if (mcp.hooks && path.startsWith(`/${mcp.slug}/hooks`)) {
				if (mcp.hooksHandler) {
					...
					res.end(
						JSON.stringify({
							error: "Hooks handler no implementado para este MCP",
						}),
					);
				}
				return;
...
server.listen(PORT, () => {
	...
	for (const mcp of mcpModules) {
		...
		if (mcp.hooksHandler && mcp.hooks && mcp.hooks.length > 0) {
