...
async function handler(
  ...
  try {
		const mcpServer = new McpServer({ name: "mcp-sonar", version: "1.0.0" });
...
export const sonarMcp: McpModule = {
	slug: "sonar",
	displayName: "MCP SonarQube",
	credentials: [
		{
			key: "SONAR_BASE_URL",
			required: false,
			description: `URL base de la instancia SonarQube (default: ${DEFAULT_BASE_URL || "https://sup.gdsas.com"})`,
		},
		{
			key: "SONAR_TOKEN",
			required: true,
			description:
				"Token de usuario/global de SonarQube usado para autenticar la Web API",
		},
	],
