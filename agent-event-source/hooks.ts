...
// export const registryHook: HookDefinition[] = [...exampleHooks, ...gitHooks];
export const registryHook: HookDefinition[] = [...gitHooks];
...
// ─── Hook Persisters ─────────────────────────────────────────────────────────
//
// External modules (e.g. db layer) can register a persister to store every
// emitted hook in durable storage. Using a registration pattern keeps hooks.ts
// free of direct database imports.

type HookPersistFn = (name: string, payload: unknown) => Promise<void>;
const hookPersisters: HookPersistFn[] = [];

/**
 * Register a function that will be called after every successful hook emit.
 * Use this from server.ts (or any bootstrap file) to persist hooks to a DB.
 */
export function registerHookPersister(fn: HookPersistFn): void {
 ...
 * Registered persisters are called fire-and-forget after delivery.
 */
export async function emit(name: string, payload: unknown): Promise<void> {
	...
	for (const persist of hookPersisters) {
		persist(name, payload).catch((err) =>
			logger.error(`[hook] persister error for ${name}: ${err}`),
		);
	}
}
